...
// Helper: Tra cứu tên màu (Named Colors)
static bool GetNamedColor(string name, Color& outColor)
{
    transform(name.begin(), name.end(), name.begin(), ::tolower);
    // Danh sách màu rút gọn (Bạn có thể giữ lại danh sách đầy đủ của bạn nếu muốn)
    static const map<string, DWORD> colorMap = {
        ...
        {"none", 0}, {"transparent", 0}
    };

    auto it = colorMap.find(name);
    if (it != colorMap.end()) {
        if (name == "none" || name == "transparent") {
            outColor = Color(0, 0, 0, 0);
            return true;
        }
        DWORD hex = it->second;
        outColor = Color(255, (hex >> 16) & 0xFF, (hex >> 8) & 0xFF, hex & 0xFF);
        return true;
    }
...
void SVGElement::Parse(xml_node<>* node)
{
    ...
    if (auto attr = node->first_attribute("transform")) {
        ...
        while (pos < t.length()) {
            ...
            vector<float> vals; GetNumbers(args, vals);

            if (command == "translate" && vals.size() >= 1) transform.Translate(vals[0], (vals.size() > 1) ? vals[1] : 0, MatrixOrderPrepend);
            else if (command == "rotate" && vals.size() >= 1) transform.Rotate(vals[0], MatrixOrderPrepend);
            else if (command == "scale" && vals.size() >= 1) transform.Scale(vals[0], (vals.size() > 1) ? vals[1] : vals[0], MatrixOrderPrepend);
            else if (command == "matrix" && vals.size() >= 6) {
                // GDI+ Matrix constructor nhận 6 giá trị: m11, m12, m21, m22, dx, dy
                // SVG Matrix: a, b, c, d, e, f tương ứng m11, m12, m21, m22, dx, dy
    ...
    if (auto attr = node->first_attribute("stroke-linecap")) {
        ...
        else if (s == "square") strokeLineCap = LineCapSquare;
...
void SVGElement::InheritFrom(const SVGElement& parent)
{
    ...
    // KẾ THỪA MÀU (Nếu chưa có màu, lấy màu cha)
    if (this->fillType == FillType::Unset && parent.fillType != FillType::Unset)
    {
        ...
        this->fillRadialGradient = parent.fillRadialGradient; // Nhớ dòng này!
    }
...
Brush* SVGElement::CreateFillBrush(const RectF& bounds)
{
    ...
    if (fillType == FillType::Unset) {
        // Mặc định SVG là màu đen nếu không nói gì
    ...
    if (fillType == FillType::RadialGradient && fillRadialGradient) {
        return fillRadialGradient->CreateBrush(bounds);
    }
